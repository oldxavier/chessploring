...
def choose_next_move(board, to_move):
    ...
    for move in moves:
        ...
        for i in range(no_of_searches):
            this_no_of_wins += recursive_mc_search(this_board, first_to_move, to_win)
...
def recursive_mc_search(mc_board, to_move, to_win):
    moves = possible_moves.possible_moves(mc_board, to_move)
    if moves == None:
    ...
    # new_board = make_move(mc_board, new_move[0], new_move[1])
